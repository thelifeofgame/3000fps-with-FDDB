...
//        // draw bounding box
		...
		for (int j = 0; j < noses.size(); j++)
		{
			...
			//circle( frame, center, radius, Scalar( 0, 255, 0 ), 4, 8, 0 )
				// Overlay moustache
				int w = noses[j].width;
				int h = noses[j].height;
			
				int x = boundingbox.start_x + noses[j].x;
				int y = boundingbox.start_y + noses[j].y;
				rectangle(img, cvPoint(x,y),
					cvPoint(x + w, y +h), Scalar(255, 255, 255), 1, 8, 0);
